...
/// <summary>
/// Exports documents to Excel (.xlsx) format using ClosedXML.
/// </summary>
public class ExcelExporter : IExcelExporter
{
    /// <inheritdoc />
    public string ExportToExcel(Document document)
    {
        var xlWbook = BuildWorkbook(document, true);
    ...
    private static XLWorkbook BuildWorkbook(Document document, bool includeHeader)
    {
        var xlWbook = new XLWorkbook();

        foreach (var worksheet in document.Tables)
            AddWorksheet(xlWbook, worksheet, includeHeader);

        return xlWbook;
    }

    private static void AddWorksheet(XLWorkbook xlWbook, Table.Models.Table worksheet, bool includeHeader)
    {
        var rowIndex = 1;
        ...
        AddContentHeader(xlWorksheet, rowIndex, worksheet);

        rowIndex++;

        foreach (var row in worksheet.Content.Rows)
        {
            AddValuesRow(xlWorksheet, rowIndex, row.Cells, null);
            rowIndex++;
        }
    }

    private static void AddContentHeader(IXLWorksheet xlWorksheet, int index, Table.Models.Table worksheet)
    {
        var style = new Style
        {
    ...
    private static void AddWorksheetText(IXLWorksheet xlWorksheet, int xlRowNumber, string text, Style style)
    {
        ...
        AddValuesRow(xlWorksheet, xlRowNumber, cell, style);
    }
    ...
    private static void AddValuesRow(IXLWorksheet xlWorksheet, int xlRowNumber, List<Cell> cells, Style? style)
    {
        ...
        foreach (var xlColumn in xlColumns)
        {
            ...
            SetCellStyle(xlColumn.Cell(xlRowNumber), style);

            index++;
        }
    }

    private static void SetCellStyle(IXLCell cell, Style? style)
    {
